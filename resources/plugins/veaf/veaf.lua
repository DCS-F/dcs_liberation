...
if ctld then
    ...
    ctld.autoInitializeAllHumanTransports = function()
        ...
        local TransportTypeNames = {"Mi-8MT", "UH-1H", "UH-60L", "Mi-24P", "Yak-52", "Ka-50"}
        for name, unit in pairs(mist.DBs.humansByName) do
    ...
    ctld.autoInitializeAllLogistic = function()
        ...
        local ApcTypeNames = {"M 818", "AAV7", "M-113", "LAV-25", "Sd_Kfz_7", "Blitz_36-6700A", "Land_Rover_101_FC", "Land_Rover_109_S3", "Ural-375", "KAMAZ Truck"}
            ...
            if unit then 
                ...
                for _, apcTypeName in pairs(ApcTypeNames) do
                    if apcTypeName:lower() == unit.type:lower() then
                        ...
                        veaf.loggers.get(ctld.Id):debug(string.format("Adding CTLD infantry spawning unit %s of group %s", unit.unitName, unit.groupName))
                    end
                end
            end
        end
        veaf.loggers.get(ctld.Id):trace("ctld.logisticUnits=%s", veaf.p(ctld.logisticUnits))
        veaf.loggers.get(ctld.Id):trace("ctld.infantryUnits=%s", veaf.p(ctld.infantryUnits))
    end

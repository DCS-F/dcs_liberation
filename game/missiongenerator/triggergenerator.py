...
class TriggerGenerator:
    ...
    def _generate_clear_statics_trigger(self, scenery_clear_zones: List[Point]) -> None:
        for zone_center in scenery_clear_zones:
            ...
            # clear_trigger.add_action(MessageToAll(text=String("Clearing"),))
    ...
    def _generate_capture_triggers(
        ...
        for cp in self.game.theater.controlpoints:
            if isinstance(cp, self.capture_zone_types):
                ...
                capture_trigger.add_condition(TimeAfter(seconds=120))
                ...
                recapture_trigger.add_condition(TimeAfter(seconds=180))
                script_string = String(
